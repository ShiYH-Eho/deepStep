...
#netWay = readXml('../../data/map/net_way_new.xml')
#nodeMap = readXml('../../data/map/node.xml')
wayNet = readXml('../../data/map/way_net_new.xml')
#netWayInfo = readNetWay(netWay)
wayNameInfo = readWayName(wayMap)
#wayNodeInfo = readWayNode(wayMap)
wayNetInfo = readWayNet(wayNet)
#nodeInfo = readNode(nodeMap)

netWayInfo = [[] for i in range(W)]
for i in range(W):
	netWayInfo[i] = [[] for j in range(H)]
for wayId in wayNetInfo:
	for net in wayNetInfo[wayId]:
		x = net[0]
		y = net[1]
		if wayId not in netWayInfo[x][y]:
			netWayInfo[x][y].append(wayId)
roadSpeedList = {}

savePath = '../../data/traffic_%dmin_%d*%d_new' % (timeUnit / 60,H,W)
...
def orderToSpeed(orderList,startTime):
	...
	wayId = getWayByPos(newList[0][3],newList[0][4],netWayInfo)
	timeStamp = t
	targetWay = wayId
	wayCount = {}
	for i in range(len(newList) - 1):
		...
		wayId = getWayByPos(newList[i][3],newList[i][4],netWayInfo)
...
def dataToTraffic(filename):
	...
	for line in f.readlines():
		...
		wayList = getWayByPos(float(data[3]),float(data[4]),netWayInfo)
		if len(wayList) == 0:
			continue
		if t < 0:
			t = 0
